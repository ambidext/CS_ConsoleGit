...
namespace Font
{
    class Program
    {
        ...
        static void fillOneFont(ulong ulFont, char[,] ch, int startIdx)
        {
            string hexStr = String.Format("{0:X16}", ulFont); 

            for (int i = 0; i < fSize; i++)
            {
                string twoCh = hexStr.Substring(i * 2, 2);

                string binStr = Convert.ToString(Convert.ToByte(twoCh, 16), 2);
                for (int b = fSize - binStr.Length; b > 0; b--)
                    binStr = "0" + binStr;

                for (int b = 0; b < fSize; b++)
                {
                    int x = b + startIdx * (fSize + 1);
                    if (binStr[b] == '0')
                        ch[i, x] = '-';
                    else
                        ch[i, x] = '*';
        ...
        static void DisplayFont(char[,] ch, int width)
        {
            for (int i = 0; i < fSize; i++)
            {
        ...
        static void DisplayFontAfterTime(char[,] ch, int width, int sec)
        {
            for (int i = 0; i < fSize; i++)
            {
                for (int j = sec; j < sec+width; j++)
                {
                    Console.Write(ch[i, j%width]);
                }
                Console.WriteLine();
            }
        }
        ...
        {
            int width = (fSize + 1) * input.Length;
            char [,] ch = new char[fSize, width];
            for (int i=0; i<input.Length; i++)
            {
                fillOneFont(font[input[i]], ch, i);
            }

            DisplayFont(ch, width);
        }

        static void DisplayAfterTime(string input, int sec)
        {
            int width = (fSize + 1) * input.Length;
            char[,] ch = new char[fSize, width];
            for (int i = 0; i < input.Length; i++)
